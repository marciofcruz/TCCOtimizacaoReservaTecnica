...
serie_treino <- base_treino$QTDE_FALTAS
serie_teste <- base_teste$FALTAS
...
# Ensaio 1
modelo_manual <- arima(serie_treino, order = c(7,0,0), 
                fixed = c(0, NA, 0, 0, 0, 0, NA,
                          NA), method = c("ML"))
analisar_modelo('AR(2,7)', modelo_manual)
buscar_melhor_lag('AR(2,7)',  serie_teste, modelo_manual, 1, nrow(base_teste), T)
...
base_teste["FALTAS_MEDIA"] <- media_serie

saveRDS(modelo_manual, file = 'models/model_ts_faltas.Rds')

paste('Predição Modelo: MAPE: ',mape(base_teste$FALTAS, base_teste$FALTAS_PRED), "MAE:", mae(base_teste$FALTAS, base_teste$FALTAS_PRED))
paste('Compararação com Média: MAPE: ',mape(base_teste$FALTAS, base_teste$FALTAS_MEDIA), "MAE:", mae(base_teste$FALTAS, base_teste$FALTAS_MEDIA))
